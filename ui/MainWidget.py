from PyQt5.QtWidgets import QMainWindow, QBoxLayout
...
class SendActivationEmailTimeoutWorker(QObject):
    ...
    def run(self):
        while (
            self.main_widget.send_activation_email_btn_timeout > 0
            and not self.main_widget.user["isActivated"]
        ):
...
class MainWidget(QMainWindow):
    def __init__(self):
        ...
        self.ui.invisible_label.setProperty("is_always_shown", True)
        ...
        self.ALL_BOTTOM_OPTION_BTNS = [
            self.ui.registration_btn,
            self.ui.phone_signin_btn,
            self.ui.email_signin_btn,
        ]
        ...
        self.FORM_LINEEDIT_ERRORS_OBJ[FormLineEdit.first_name.value] = (
            self.ui.name_error_label
        )
        self.FORM_LINEEDIT_ERRORS_OBJ[FormLineEdit.surname.value] = (
            self.ui.surname_error_label
        )
        self.FORM_LINEEDIT_ERRORS_OBJ[FormLineEdit.phone.value] = (
            self.ui.phone_error_label
        )
        self.FORM_LINEEDIT_ERRORS_OBJ[FormLineEdit.email.value] = (
            self.ui.email_error_label
        )
        self.FORM_LINEEDIT_ERRORS_OBJ[FormLineEdit.password.value] = (
        ...
        self.FORM_LINEEDIT_LAYOUTS_TO_LINEEDIT_OBJ[self.ui.name_layout] = (
            self.ui.name_lineedit
        )
        self.FORM_LINEEDIT_LAYOUTS_TO_LINEEDIT_OBJ[self.ui.surname_layout] = (
            self.ui.surname_lineedit
        )
        self.FORM_LINEEDIT_LAYOUTS_TO_LINEEDIT_OBJ[self.ui.phone_layout] = (
            self.ui.phone_lineedit
        )
        self.FORM_LINEEDIT_LAYOUTS_TO_LINEEDIT_OBJ[self.ui.email_layout] = (
            self.ui.email_lineedit
        )
        self.FORM_LINEEDIT_LAYOUTS_TO_LINEEDIT_OBJ[self.ui.password_layout] = (
        ...
        self.BOTTOM_OPTION_BTNS_OBJ[AuthOption.register.value] = (
            self.ui.registration_btn
        )
        ...
        self.FORM_LINEEDITS_VARIANTS[AuthOption.register.value] = (
            self.ALL_LINEEDIT_LAYOUTS
        )
        self.FORM_LINEEDITS_VARIANTS[AuthOption.phone.value] = [
            self.ui.phone_layout,
            self.ui.password_layout,
        ]
        self.FORM_LINEEDITS_VARIANTS[AuthOption.email.value] = [
            self.ui.email_layout,
            self.ui.password_layout,
        ]
        ...
        self.ui.activation_code_lineedit.textChanged.connect(
            self.activation_code_lineedit_textChanged
        )
    ...
    def preserve_curr_variant_lineedit(self):
        for lineedit_layout in self.FORM_LINEEDITS_VARIANTS[self.selected_auth_option]:
            ...
            form_lineedit_enum_value = list(
                filter(
                    lambda x: x[1] == lineedit, list(self.FORM_LINEEDITS_OBJ.items())
                )
            )[0]
    ...
    def render_form_lineedits_curr_variant(self):
        if self.shown_window == ShownWindowEnum.form.value:
            for lineedit_layout in self.ALL_LINEEDIT_LAYOUTS:
                if lineedit_layout in self.FORM_LINEEDITS_VARIANTS.get(
                    self.selected_auth_option
                ):
    ...
    def render_form_bottom_btns(self):
        if self.shown_window == ShownWindowEnum.form.value:
            ...
            self.ui.submit_btn.setText(
                self.SUBMIT_BTN_TEXT_VARIANTS.get(self.selected_auth_option)
            )
        ...
        if self.is_auth:
            if self.user["isActivated"]:
                ...
                if self.send_activation_email_btn_timeout:
                    self.ui.send_activation_email_btn.setDisabled(True)
                    self.ui.send_activation_email_btn.setCursor(QCursor(Qt.ArrowCursor))
                    self.ui.send_activation_email_btn.setText(
                        f"Wait {self.send_activation_email_btn_timeout} seconds to send another one"
                    )
                else:
                    self.ui.send_activation_email_btn.setDisabled(False)
                    self.ui.send_activation_email_btn.setCursor(
    ...
    def show_profile(self):
        ...
        self.ui.main_layout.setDirection(QBoxLayout.Direction.RightToLeft)
        self.ui.main_layout.setSpacing(22)  # shift profile a bit

        if self.is_auth:
            # replace placeholder text with user's name and surname
            self.ui.welcome_label.setText(
                self.ui.welcome_label.text().replace(
                    consts.PROFILE_FULLNAME_PLACEHOLDER,
    ...
    def show_form(self):
        ...
        self.ui.main_layout.setDirection(QBoxLayout.Direction.LeftToRight)
    ...
    def validate_lineedits(self):
        ...
        def validate_phone_lineedit(phone):
            ...
            is_validator_valid = (
                self.phone_lineedit_validator.validate(phone, 0)[0] != 1
            )
        ...
        def validate_password_lineedit(password):
            ...
            is_allowed_password = (
                does_contain_lowercase_letter
                and does_contain_uppercase_letter
                and does_contain_2_or_more_digits
                and does_contain_special_char
                and is_allowed_length
            )
        ...
        validate_field(
            self.ui.name_lineedit,
            get_custom_validator(self.name_lineedit_validator),
            FormLineEdit.first_name.value,
            consts.NAME_ERROR_MSG,
        )

        validate_field(
            self.ui.surname_lineedit,
            get_custom_validator(self.surname_lineedit_validator),
            FormLineEdit.surname.value,
            consts.SURNAME_ERROR_MSG,
        )

        validate_field(
            self.ui.phone_lineedit,
            validate_phone_lineedit,
            FormLineEdit.phone.value,
            consts.PHONE_ERROR_MSG,
        )

        validate_field(
            self.ui.email_lineedit,
            get_custom_validator(self.email_lineedit_validator),
            FormLineEdit.email.value,
            consts.EMAIL_ERROR_MSG,
        )

        validate_field(
            self.ui.password_lineedit,
            validate_password_lineedit,
            FormLineEdit.password.value,
            consts.PASSWORD_ERROR_MSG,
        )
                ...
                if is_valid_form:
                    ...
                    try:
                        if self.selected_auth_option == AuthOption.register.value:
                            response = register(
                                self.ui.name_lineedit.text().strip(),
                                self.ui.surname_lineedit.text().strip(),
                                self.ui.phone_lineedit.text().strip(),
                                self.ui.email_lineedit.text().strip(),
                    ...
                    if response:
                        if response["success"]:
                            ...
                            self.form_errors[self.selected_auth_option] = response[
                                "data"
                            ]
                            self.render_form_lineedit_errors()
            finally:
    ...
    def send_activation_email(self):
        ...
        has_sent_msg_successfully = send_account_activation_message(
            self.user_address["email"], self.acc_activation_code
        )
        ...
        self.send_activation_email_btn_timeout = (
            consts.SEND_ACTIVATION_EMAIL_BTN_TIMEOUT_S
        )

        def set_activation_email_timeout():
            self.send_activation_email_btn_timeout_thread = QThread()
            self.send_activation_email_btn_timeout_worker = (
                SendActivationEmailTimeoutWorker(self)
            )

            self.send_activation_email_btn_timeout_worker.moveToThread(
                self.send_activation_email_btn_timeout_thread
            )

            self.send_activation_email_btn_timeout_thread.started.connect(
                self.send_activation_email_btn_timeout_worker.run
            )

            self.send_activation_email_btn_timeout_worker.finished.connect(
                self.send_activation_email_btn_timeout_thread.quit
            )
            self.send_activation_email_btn_timeout_worker.finished.connect(
                self.send_activation_email_btn_timeout_worker.deleteLater
            )
            self.send_activation_email_btn_timeout_thread.finished.connect(
                self.send_activation_email_btn_timeout_thread.deleteLater
            )
    ...
    def activation_code_lineedit_textChanged(self, event):
        if (
            self.ui.activation_code_lineedit.text().replace(" ", "")
            == self.acc_activation_code
        ):
            try:
